...
<body link="#1D9BF0" alink="#1D9BF0" vlink="#1D9BF0" style="background-color: rgb(21, 32, 43);">
  ...
  echo '<form action="./upfile_image.php" method="POST" enctype="multipart/form-data">';
  echo '<span>名前 </span><input type="text" size="10" autocomplete="name" name="name" value="';
echo $_COOKIE['username'];
echo '"><span> </span><input type="file" name="file" accept="image/*" onchange="previewFile(this);" required>';
echo '<input type="submit" value="画像をアップロードする"></form>';
...
echo '<h2>UP完了</h2>';
		//ログに記録
        $putting='<div style="border: 2px solid white;padding-left:10px;padding-bottom:10px"><p><span style="color:white">'.$_COOKIE['username'].'</span><a style="color:white" href="'."./receive/".$count.$filetype.'"target="_blank" rel="noopener noreferrer"><img src="'."./receive/".$count.$filetype.'"width=200px></a><span style="color:white">'."  ".$todaydate.'</span>'."</p></div>"."\n";
        $count++;
